...
public class test
{
    private static final int QRCOLOR = 0xFF0000FF;   //藍色
    private static final int BGWHITE = 0xFFFFFFFF;   //背景色
    ...
    {  
        try  
        {  
            getLogoQRCode("http://member.must.org.tw:8080/MUST/CHECKIN/INDEX.jsp?member_no=U0002", "測試QR CODE");
        }  
        catch (Exception e)  
        {  
            e.printStackTrace();  
...
//      String filePath = (javax.servlet.http.HttpServletRequest)request.getSession().getServletContext().getRealPath("/") + "resources/images/logoImages/mustlogo.png";
        String filePath = "C:/mustlogo.png";  //TODO  
    ...
    public String addLogo_QRCode(BufferedImage bim, File logoPic, LogoConfig logoConfig, String productName)
    {
        try
        {
             ...
             * 設置logo的大小
             */
            int widthLogo = logo.getWidth(null)>image.getWidth()*25/100?(image.getWidth()*25/100):logo.getWidth(null), 
                heightLogo = logo.getHeight(null)>image.getHeight()*25/100?(image.getHeight()*25/100):logo.getWidth(null);
...
//                  outg.drawString(productName, 0, image.getHeight() + (outImage.getHeight() - image.getHeight())/2 + 5 ); //画文字
            ...
            ImageIO.write(image, "png", new File("C:/TDC-" + new Date().getTime() + "test.png")); //TODO  
