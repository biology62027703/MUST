...
public class MAIL_TSO1C01 extends MAIL_TEMPLATE {
	...
	public String getMailSubject() {
		...
		String mail_subject = "《司法院線上起訴及書狀傳送作業平台》案件資料更新通知信(案號:"+check.checkNull(param.get("CRMYY"), "").toString()+"年"+check.checkNull(param.get("CRMID"), "").toString()+"字"+check.checkNull(param.get("CRMNO"), "").toString()+"號)";
		if( check.checkNull(param.get("CRMYY"), "").toString().equals("") )
			mail_subject = "《司法院線上起訴及書狀傳送作業平台》案件資料更新通知信";
		return mail_subject;
	}
