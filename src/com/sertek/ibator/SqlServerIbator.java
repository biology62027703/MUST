 ...
 *
 */
public class SqlServerIbator {
	
	private Logger logger = Logger.getLogger(this.getClass());
	private DBUtility db;
	private StringBuffer sql = new StringBuffer();
	private String owner = "so.";
	private String workspace = "D:\\Workspaces\\��L(�u�W�_�D)\\SOL\\";
	private String javaModelDir = workspace + "src\\com\\sertek\\table\\";
	private String sqlMapXmlDir = workspace + "WebRoot\\WEB-INF\\ibatis-xml\\table\\";
	private String javaModelExtendsClass = "com.sertek.table.BaseTable";
	private String javaModelPackage = "com.sertek.table";
	private boolean isTempOwner = false;
	private boolean isBakFile = false;
	private boolean withNolock = true;
	
	private Map ignoredTable = new HashMap();
	private Map bosTable = new HashMap();
	private Map noRowidTable = new HashMap();
	...
	public SqlServerIbator(DBUtility db, String owner) {
		this.db = db;
		this.owner = owner.toLowerCase();

		if (owner.toUpperCase().startsWith("TEMP")) {
			this.isTempOwner = true;
			this.owner = "tempjd.";
			this.javaModelDir = workspace + "src\\com\\sertek\\table\\temp\\";
			this.sqlMapXmlDir = workspace + "WebRoot\\WEB-INF\\ibatis-xml\\temp\\";
			this.javaModelPackage = "com.sertek.table.temp";
		} else if (owner.toUpperCase().startsWith("F")) {
			this.isTempOwner = false;
			this.owner = "fjd.";
			this.javaModelDir = workspace + "src\\com\\sertek\\table\\fjd\\";
			this.sqlMapXmlDir = workspace + "WebRoot\\WEB-INF\\ibatis-xml\\fjd\\";
			this.javaModelPackage = "com.sertek.table.fjd";
		}

		ignoredTable.put("S03", "");
		ignoredTable.put("R04_del", "");
		ignoredTable.put("SUITKD_err", "");
		
		bosTable.put("S61", "");
		bosTable.put("S6A", "");
		bosTable.put("S6B", "");
		
		noRowidTable.put("SUITKD", "");
		noRowidTable.put("Y60", "");
		noRowidTable.put("Y61", "");
		noRowidTable.put("Y62", "");
		noRowidTable.put("Y6A", "");
		noRowidTable.put("Y6B", "");
...
//		crmSysidTable.put("C6B", "");
	...
	public void doGenerateTable(String tableName) {
		try {
			if (ignoredTable.containsKey(tableName)) {
				logger.info("ignoredTable [" + tableName + "] ���۰ʲ���");
				return;
			}
			
			if (bosTable.containsKey(tableName)) {
				owner = "bos.";
			} else {
				owner = "so.";
			}
	...
	private void genSqlMapXml(Table table) {
		...
		if (!noRowidTable.containsKey(table.getTableName())) {
	    ...
	    content.add("-->");
	    if (isTempOwner) {
			content.add("<sqlMap namespace=\"TEMP_" + table.getTableName() + "\">");
		} else {
		...
		content.add("		from " + owner + "." + table.getTableName().toLowerCase() + (withNolock ? " with (nolock)" : ""));
		...
		content.add("	<insert id=\"cloneTable\" parameterClass=\"" + hashMapObj + "\">");
		content.add("		insert into $toTable$");
		content.add("			(" + allInsertColumn + ")");
		content.add("		select " + allInsertColumn);
		content.add("		from $fromTable$");
		content.add("		where crmyy = #crmyy#");
		content.add("		  and crmid = #crmid#");
		content.add("		  and crmno = #crmno#");
		content.add("	</insert>");
	...
	private void genJavaModel(Table table) {
		...
		HashMap encMap = this.getEncColumnMap(table.getTableName());
		...
		for (int i = 0; i < columnList.size(); i++) {
			...
			boolean isEnc = encMap.containsKey(column.getColumnName().toUpperCase());
			...
			if (isEnc) {
				getterAndSetterMethod.add("		return decrypt(" + column.getColumnName().toLowerCase() + ");");
			} else {
	...
	public List getTables() {
		...
		sql.append("select name from " + owner.toLowerCase() + "sys.tables order by name");
		logger.info(sql);
...
//		}
		...
		sql.append(owner.toLowerCase() + "sys.columns ");
		sql.append("where object_id = (select object_id from " + owner.toLowerCase() + "sys.tables where name = '" + table.getTableName() + "') ");
		sql.append("and name <> 'rowid' ");
		sql.append("order by column_id");
		logger.info(sql);
		Vector vt = db.doSqlSelect(sql.toString(), columnNum, false);
		for (int i = 0; i < vt.size() / columnNum; i++) {
...
//		}
	...
	private HashMap getEncColumnMap(String tblnm) {
		HashMap encMap = new HashMap();
		int columnNum = 1;
		sql.setLength(0);
		sql.append("select colnm from ");
		sql.append("SO..ENSET ");
		sql.append("where tblnm = '" + tblnm + "' ");
		logger.info(sql);
		Vector vt = db.doSqlSelect(sql.toString(), columnNum, false);
		for (int i = 0; i < vt.size() / columnNum; i++) {
			String colnm = vt.get(i * columnNum + 0).toString().toUpperCase();
			encMap.put(colnm, colnm);
		}
		return encMap;
	}
}
