...
HelpUtil.prototype.showInfoBar = function() 
{
	...
	$("#img1", $objInfoBar).attr("src", helpUtil.getWebRoot()+"resources/image/"+strInfoType.toLowerCase()+".gif");
...
HelpUtil.prototype.showProcessBar = function() 
{
	...
	$("#img2", $objProcessBar).attr("src", helpUtil.getWebRoot()+"resources/image/loading.gif");
	$("#img3", $objProcessBar).attr("src", helpUtil.getWebRoot()+"resources/image/cancel.gif");
	...
	$("#___HELP_PROCESSBAR___").css("top", document.body.scrollTop);
	$("#___HELP_PROCESSBAR___").css("left", document.body.scrollLeft);
}
...
HelpUtil.prototype.showWindowMask = function()
{
	...
	$("#___HELP_WINDOWMASK___").css("top", document.body.scrollTop);
	$("#___HELP_WINDOWMASK___").css("left", document.body.scrollLeft);
}
...
document.writeln("	<td style='width:32px'><img id='img1' src='"+helpUtil.getWebRoot()+"resources/image/warn.gif' style='width:32px; height: 32px; vertical-align:middle'></td>");
document.writeln("	<td><font id='font1' style='font:bold 17pt 新細明體; color: #996600'>111</font></td>");
document.writeln("	<td style='width:32px'><img src='"+helpUtil.getWebRoot()+"resources/image/cancel.gif' alt='取消' style='float:right; cursor: pointer;width:32px; height: 32px; vertical-align:middle' onClick='helpUtil.closeInfoBar()'></td>");
...
document.writeln("	<img id='img2' src='"+helpUtil.getWebRoot()+"resources/image/loading.gif' style='float:left;width:32px; height: 32px; vertical-align:middle'>");
document.writeln("	<font id='font2' style='float:left; margin-top: 5px; font:bold 17pt 新細明體;'>資料處理中, 請稍待...</font>");
document.writeln("	<img id='img3' src='"+helpUtil.getWebRoot()+"resources/image/cancel.gif' alt='取消' style='float:right; cursor: pointer;width:32px; height: 32px; vertical-align:middle' onClick='helpUtil.closeProcessBar()'>");
document.writeln("</div>");
document.writeln("<div id='___HELP_WINDOWMASK___' style='position:absolute; top: 0px; left: 0px; z-index: 1000; width: 100%; height: 100%; padding:0px 0px 0px 0px; margin:0px 0px 0px 0px; background-color:#E8FCE4; filter: Alpha(Opacity=50); overflow: hidden;visibility:hidden'>")
